...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.execute)('INSERT INTO `groups` (name, description, course_id, grade_id, max_students, is_active) VALUES (?, ?, ?, ?, ?, ?)', [name, description || null, course_id, finalGradeId, max_students || 30, is_active]);
        const newGroup = await (0, db_1.queryOne)('SELECT * FROM `groups` WHERE id = ?', [result.insertId]);
