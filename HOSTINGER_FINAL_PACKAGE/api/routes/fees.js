...
const ensureFeesSchema = async () => {
    ...
    for (const [column, alterStmt] of Object.entries(requiredColumns)) {
        const columnExists = await (0, db_1.queryOne)(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'student_fees' AND COLUMN_NAME = ?`, [DATABASE_NAME, column]);
        if (!columnExists) {
            await (0, db_1.execute)(`ALTER TABLE student_fees ${alterStmt}`);
        }
    }
...
router.get('/', async (req, res) => {
    try {
        ...
        const fees = await (0, db_1.query)(sql, params);
        res.json(fees);
    }
    catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
        const fee = await (0, db_1.queryOne)('SELECT * FROM student_fees WHERE id = ?', [req.params.id]);
        if (!fee) {
...
router.post('/', async (req, res) => {
    try {
        await ensureFeesSchema();
        const { student_name, phone, grade_id, grade_name, group_id, group_name, barcode, amount, paid_amount, status, payment_method, is_offline, notes, due_date, payment_date, receipt_image_url } = req.body;
        if (!student_name && !phone && !barcode) {
            return res.status(400).json({ error: 'Student information is required' });
        }
        let linkedStudent = null;
        if (barcode) {
            linkedStudent = await (0, db_1.queryOne)(`SELECT s.*, g.name AS grade_name, grp.name AS group_name
                 FROM students s
        ...
        const result = await (0, db_1.execute)(`INSERT INTO student_fees (
        ...
        let newFee = null;
        if (result.insertId) {
            newFee = await (0, db_1.queryOne)('SELECT * FROM student_fees WHERE id = ?', [result.insertId]);
        }
        if (!newFee) {
            newFee = await (0, db_1.queryOne)(`SELECT * FROM student_fees
                 WHERE barcode = ?
                 ORDER BY created_at DESC
                 LIMIT 1`, [resolvedBarcode]);
        }
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { student_name, phone, grade_id, grade_name, group_id, group_name, amount, paid_amount, status, payment_method, notes, due_date, payment_date, receipt_image_url } = req.body;
        await (0, db_1.execute)(`UPDATE student_fees SET
        ...
        const updatedFee = await (0, db_1.queryOne)('SELECT * FROM student_fees WHERE id = ?', [id]);
        res.json(updatedFee);
    }
    catch (error) {
...
router.delete('/:id', async (req, res) => {
    try {
        const result = await (0, db_1.execute)('DELETE FROM student_fees WHERE id = ?', [req.params.id]);
