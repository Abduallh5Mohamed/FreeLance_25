  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!phoneRegex.test(formData.phone)) {
        toast({
          title: "خطأ في التحقق",
          description: "الرجاء إدخال رقم هاتف صحيح (يبدأ ب 010, 011, 012, أو 015 ويتكون من 11 رقم)",
          variant: "destructive"
        });
        setLoading(false);
        return;
      ...
      const { data: existingEmail } = await supabase
        ...
        .neq('id', editingStudent?.id || '00000000-0000-0000-0000-000000000000');
      ...
      // Hash password - required for new students
      let passwordHash = null;
      if (formData.password && formData.password.trim()) {
        passwordHash = await bcrypt.hash(formData.password.trim(), 10);
        console.log('✅ Password hashed successfully for:', formData.email);
      } else if (!editingStudent) {
        // Password is required for new offline students
        toast({
          title: "خطأ",
      ...
      // Calculate total subscription cost from selected courses
      const totalSubscriptionCost = selectedCourses.reduce((total, courseId) => {
        const course = courses.find(c => c.id === courseId);
        return total + Number(course?.subscriptions?.price || 0);
      }, 0);
      ...
      if (editingStudent) {
        ...
        // Record subscription payment in account statement
        if (totalSubscriptionCost > 0) {
          await supabase
            ...
            .insert({
              ...
              description: `اشتراك الطالب ${formData.name} - ${selectedCourses.length} كورس`
            });
        ...
        toast({
          title: "تم الإضافة",
          description: "تم إضافة الطالب الأوفلاين بنجاح. يمكنه الدخول بالإيميل والباسورد.",
        });
      }
