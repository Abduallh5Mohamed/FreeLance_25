...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    if (userStr) {
      ...
      // Connect Socket
      // Assuming API is at /api, so socket is at root /
      const newSocket = io('/', {
      ...
      newSocket.on('message:new', (message: Message) => {
        ...
        // This is a simplified update, in a real app you'd refetch or update state intelligently
        fetchConversations(); 
      });
  ...
  // Fetch Conversations (Mock implementation based on existing patterns if API is missing)
  const fetchConversations = async () => {
    // In a real implementation, you would call axios.get('/api/messages/conversations')
    // For now, I'll assume we can fetch available users or use a mock if API fails
    try {
  ...
  useEffect(() => {
    if (selectedUser) {
      // Mark as read
      // fetch(`/api/messages/${selectedUser.id}/mark-read`, { method: 'PUT' ... })
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col" dir="rtl">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className={`
          ${isMobile ? (selectedUser ? 'hidden' : 'w-full') : 'w-1/3 min-w-[320px] max-w-[400px] border-l'}
          bg-white dark:bg-slate-800 flex flex-col h-[calc(100vh-64px)]
        `}>
          {/* Sidebar Header */}
          <div className="p-4 border-b bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
              <span className="text-primary">الرسائل</span>
              <span className="text-sm font-normal text-muted-foreground bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded-full">
        ...
        {/* Chat Area */}
        <div className={`
          ${isMobile ? (selectedUser ? 'flex' : 'hidden') : 'flex-1 flex'}
          flex-col bg-slate-50/50 dark:bg-slate-900/50 h-[calc(100vh-64px)] relative
        `}>
          {selectedUser ? (
            <>
              {/* Chat Header */}
              <div className="p-3 border-b bg-white dark:bg-slate-800 flex items-center gap-3 shadow-sm z-10">
                {isMobile && (
                        ...
                        return (
                            ...
                            >
                                <div
                                    ...
                                    `}
                                >
                                    <p className="text-sm leading-relaxed whitespace-pre-wrap dir-auto" dir="auto">
                                        {msg.content}
                                    </p>
              ...
              {/* Input Area */}
              <div className="p-4 bg-white dark:bg-slate-800 border-t sticky bottom-0 z-20">
                <div className="max-w-3xl mx-auto flex items-end gap-2">
                  <Button variant="ghost" size="icon" className="shrink-0 text-muted-foreground">
                    <ImageIcon className="h-5 w-5" />
                  </Button>
                  <div className="flex-1 bg-slate-100 dark:bg-slate-900 rounded-2xl border focus-within:ring-2 focus-within:ring-primary/20 transition-all flex items-center px-4 py-2">
                    <textarea
                        value={inputText}
                        ...
                        className="w-full bg-transparent border-none focus:outline-none resize-none max-h-32 min-h-[44px] py-2 text-sm"
                  ...
                  <Button 
                    ...
                    className="shrink-0 rounded-full h-12 w-12 bg-yellow-400 hover:bg-yellow-500 text-black shadow-lg"
                  >
